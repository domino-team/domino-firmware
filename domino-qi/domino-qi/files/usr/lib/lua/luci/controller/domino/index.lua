...
function homepage()
  ...
  --node ideino
  local ideino=luci.util.exec("netstat -pl 2>/dev/null|grep node|awk '{print $4}'")
  local ideino_port
  if ideino then
	address, ideino_port=ideino:match("([^:]+):([^:]+)")
	if ideino_port then
		ideino_port=string.match(ideino_port,"%d+")
	end
  end
